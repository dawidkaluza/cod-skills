...
enum _:CVARS {
    ...
    CVAR_BURNS_NUM,
    CVAR_PERIOD_TIME,
    CVAR_INT_MULTIPLIER_IN_CHANCE,
    CVAR_INT_MULTIPLIER_IN_MOLOTOV
};
new g_pcvar[CVARS];

//TODO what if player will have more hegrenades than molotov grenades? Plugin is not prepared for this situation
public plugin_init() {
    ...
    g_pcvar[CVAR_INT_MULTIPLIER_IN_CHANCE] = register_cvar("cod_skill_burn_int_multiplier_in_chance", "0.02");
    g_pcvar[CVAR_INT_MULTIPLIER_IN_MOLOTOV] = register_cvar("cod_skill_burn_int_multiplier_in_chance", "0.2");
    ...
    if (damageBits & (1<<1)) {
        if (g_curChanceToBurn[att] && !random(g_curChanceToBurn[att])) {
            Cod_BurnPlayer(
                ...
                get_pcvar_float(g_pcvar[CVAR_DAMAGE]) + get_pcvar_float(g_pcvar[CVAR_INT_MULTIPLIER_IN_CHANCE]) * cod_get_user_intelligence(att)
            );
        }
    }
    ...
    if (damageBits & (1<<24)) {
        if (pev_valid(ent) && IsMolotov(ent)) {
            Cod_BurnPlayer(
                att, id, 
                get_pcvar_float(g_pcvar[CVAR_PERIOD_TIME]), get_pcvar_num(g_pcvar[CVAR_BURNS_NUM]), 
                get_pcvar_float(g_pcvar[CVAR_DAMAGE]) + get_pcvar_float(g_pcvar[CVAR_INT_MULTIPLIER_IN_MOLOTOV]) * cod_get_user_intelligence(att)
            );
        }
    }
